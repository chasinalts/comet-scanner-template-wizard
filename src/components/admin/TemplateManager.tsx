...
import { Template, TemplateInsert, TemplateUpdate, Section } from '@/types/supabaseDb';
...
export default function TemplateManager({ onTemplateUpdate }: TemplateManagerProps) {
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="bg-slate-700 border border-slate-600 rounded-lg p-4 max-h-60 overflow-y-auto">
                {sections.length === 0 ? (
                  <p className="text-slate-400 text-sm">No sections available. Create sections first.</p>
                ) : (
                  <div className="space-y-2">
                    {sections.map((section) => (
                      <label key={section.id} className="flex items-center space-x-2 cursor-pointer">
                        <input
                          type="checkbox"
                          checked={formData.section_ids?.includes(section.id) || false}
                          onChange={() => handleSectionToggle(section.id)}
                          className="rounded"
                        />
                        <span className="text-white text-sm">{section.title}</span>
                        <span className="text-xs text-slate-400">({section.question_type})</span>
                      </label>
                    ))}
                  </div>
                )}
            ...
            <div className="flex items-center">
              <input
                ...
                className="mr-2"
              />
              <label htmlFor="template_is_active" className="text-cyan-300 text-sm">
                Active
              </label>
            </div>
              ...
              <div className="flex justify-between items-start">
                <div className="flex-1">
                  ...
                  <div className="text-sm text-slate-400 space-y-1">
                    <div>Sections ({template.section_ids.length}): {getSectionNames(template.section_ids) || 'None'}</div>
