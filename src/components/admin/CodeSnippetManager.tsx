...
export default function CodeSnippetManager({ onSnippetUpdate }: CodeSnippetManagerProps) {
  ...
  const categories = ['utility', 'component', 'hook', 'api', 'styling', 'configuration', 'other'];
  const languages = ['javascript', 'typescript', 'html', 'css', 'json', 'python', 'bash', 'sql'];
  ...
  const fetchSnippets = async () => {
    try {
      ...
      if (error) {
        ...
        if (error.code === '42P01') {
          setError('Code snippets table not found. Please create the table first.');
          setSnippets([]);
        } else {
          throw error;
        }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to delete code snippet');
    }
  };
  ...
  const filteredSnippets = snippets.filter(snippet => {
    const categoryMatch = filterCategory === 'all' || snippet.category === filterCategory;
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex items-center">
              <input
                ...
                className="mr-2"
              />
              <label htmlFor="snippet_is_active" className="text-cyan-300 text-sm">
                Active
              </label>
            </div>
