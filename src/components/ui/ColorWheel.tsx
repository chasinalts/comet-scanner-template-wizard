...
const ColorWheel: React.FC<ColorWheelProps> = ({ onColorChange, initialColor = '#1f2937' }) => {
  // Initialise from `initialColor` (hex) if supplied
  ...
  const [saturation, setSat]  = useState(() => hexToHsl(initialColor)[1]);
  const [lightness, setLight] = useState(() => hexToHsl(initialColor)[2]);
