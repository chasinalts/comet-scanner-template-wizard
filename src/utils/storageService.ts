// Storage service abstraction layer that uses Supabase for file storage
...
export const getBucketId = (bucketType: BucketType): string => {
  return bucketMap[bucketType] || BANNER_BUCKET;
};
 ...
 */
const generateFilePath = (fileName: string, bucketType: BucketType): string => {
  const timestamp = Date.now();
  const randomString = Math.random().toString(36).substring(2, 10);
  const extension = fileName.split('.').pop();
  return `${bucketType}/${timestamp}-${randomString}.${extension}`;
};
