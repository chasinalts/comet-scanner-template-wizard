...
const mockListFiles = (bucketType: string) => {
...
const mockGetContent = (contentType: string) => {
...
const mockUploadImage = (file: File, bucketType: string, userId: string) => {
  return {
...
vi.mock('../utils/supabaseImageStorage', () => ({
  listFiles: vi.fn().mockImplementation(mockListFiles),
  uploadFile: vi.fn().mockImplementation(mockUploadImage),
  getFileUrl: vi.fn().mockImplementation((path, bucket) => `https://example.com/${bucket}/${path}`),
  getFilePreview: vi.fn().mockImplementation((id, bucket) => `https://example.com/${bucket}/preview/${id}.jpg`),
}));
  ...
  databaseService: {
    list: vi.fn().mockImplementation((collection, filters) => {
      if (collection === 'content') {
        return [{ id: 'content-1', type: 'what_is_comet', content: 'Test COMET description' }];
      }
      return [];
    }),
...
import { listFiles, uploadFile, getFileUrl, getFilePreview } from '../utils/supabaseImageStorage';
...
describe('Image and Data Flow', () => {
  ...
  describe('Image URL Generation', () => {
    ...
    it('should generate preview URLs for images', async () => {
      ...
      expect(getFilePreview).toHaveBeenCalledWith('test-id', 'banner');
  ...
  describe('Image Upload', () => {
    it('should upload an image and return metadata', async () => {
      ...
      const uploadResult = await uploadFile(mockFile, 'banner', 'user-123');
      
      expect(uploadFile).toHaveBeenCalledWith(mockFile, 'banner', 'user-123');
  ...
  describe('End-to-End Flow', () => {
    it('should simulate the flow from upload to retrieval', async () => {
      ...
      const previewUrl = await getFilePreview(bannerImages[0].id, 'banner');
