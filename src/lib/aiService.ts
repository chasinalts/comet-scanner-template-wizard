  ...
  async generateCode(request: AIRequest): Promise<AIResponse> {
    ...
    try {
      switch (request.provider) {
        case 'openai':
          generatedContent = await this.callOpenAI(request.prompt, systemPrompt);
          break;
        case 'gemini':
          generatedContent = await this.callGemini(request.prompt, systemPrompt);
          break;
        case 'claude':
          generatedContent = await this.callClaude(request.prompt, systemPrompt);
          break;
        case 'openrouter':
          generatedContent = await this.callOpenRouter(request.prompt, systemPrompt);
          break;
        default:
      ...
      const codeMatch = generatedContent.match(/```(?:pinescript|pine)?\n([\s\S]*?)\n```/);
